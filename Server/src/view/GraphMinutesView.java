...
/**
 * Class for displaying the graph with the amount of total song minutes played
 * <p>
 * @author Arcadia Youlten
 * @version %I% %G%
 */

public class GraphMinutesView extends JPanel {
    ...
    private final int numberYDivisions = 25;
    private float minMinutes = Float.MAX_VALUE;
    private float maxMinutes = Float.MIN_VALUE;

    private ArrayList<Statistics> stats;

    public GraphMinutesView(){
    ...
    private void getMinMinutes() {
        for (Statistics statistics : stats) {
            this.minMinutes = Math.min(this.minMinutes, statistics.getNumMinutes());
        }
    }
    ...
    private void getMaxMinutes() {

        for (Statistics statistics : stats) {
            this.maxMinutes = Math.max(this.maxMinutes, statistics.getNumMinutes());
        }
    }

    /**
     * This method constructs 1 graph with the provided statistics.
     * This graph displays the number total song minutes played.
     *
     * @see GraphView
     *
    ...
    private void drawHatchMarks(Graphics2D g2, List<Point> graphPoints) {
        ...
        for (int i = 0; i < numberYDivisions +1; i++) {
            ...
            if (stats.size() > 0) {
                ...
                g2.setColor(Color.BLACK);
                yLabel = ((int) ((minMinutes + (maxMinutes - minMinutes) * ((i * 1.0) / numberYDivisions)) * 100)) / 100+ "";
    ...
    private double getyScale() {
        ...
        if (maxMinutes != minMinutes)
            yScale =
                    ((double) MAIN_PANEL_HEIGHT - 3 * margin) / (maxMinutes - minMinutes);
        else
    ...
    private List<Point> convertPoints(){
        ...
        for (int i = 0; i < stats.size(); i++) {
            ...
            int y1 = (int) ((maxMinutes - stats.get(i).getNumMinutes()) * yScale + margin);
